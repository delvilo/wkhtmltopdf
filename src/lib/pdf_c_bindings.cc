 ...
 * This is done by relatively simple reflection on the CropSettings, Margin,
 * ImageGlobal, PdfGlobal, PdfObject, and Size classes.
 *
 * - The \ref wkhtmltopdf_global_settings corresponds to the PdfGlobal class and is documented in \ref pagePdfGlobal.
 * - The \ref wkhtmltopdf_object_settings corresponds to the PdfObject class and is documented in \ref pagePdfObject.
 ...
 *      - "skip" Skip the input; conversion fails because no printable input remains
...
/**
 * \brief Legacy capability query retained for C ABI compatibility
 *
 * \return Always 0; this fork uses standard Qt5/WebKit
 */
 ...
 *
 * \param use_graphics Ignored; retained for ABI compatibility. Qt5 uses the
 * offscreen platform unless QT_QPA_PLATFORM is explicitly set.
 * \returns 1 on success and 0 otherwise
 *
 * \sa wkhtmltopdf_deinit
 */
CAPI(int) wkhtmltopdf_init(int use_graphics) {
	Q_UNUSED(use_graphics); // Retained for C ABI compatibility.
	++usage;

	if (qApp == 0) {
		static char name[] = "wkhtmltox";
...
CAPI(int) wkhtmltopdf_get_global_setting(wkhtmltopdf_global_settings * settings, const char * name, char * value, int vs) {
	if (!settings || !value || vs <= 0) return 0;
...
/**
 * \brief Convert exactly one HTML input into a PDF document
 *
 ...
 *
 * Supply the single HTML input described by the object settings. Adding zero or more
 * than one object makes wkhtmltopdf_convert fail before loading or printing. Once the object has been added, the
